	...
	<view class="testing">
		...
		<scroll-view scroll-x class="bg-student nav" scroll-with-animation :scroll-left="scrollLeft">
			<view class="cu-item overviewCss"  v-for="(item,index) in studentTopData" :key="index" :data-id="index">
				...
				<view class="number" >{{index > 0 ?  `${item.num}分` : `(第${item.num}学期)`}}</view>
			</view>
		</scroll-view>
		...
		<scroll-view scroll-x class="bg-student nav" scroll-with-animation :scroll-left="scrollLeft">
			<view class="cu-item overviewNavCss"  v-for="(item,index) in studentData" :key="index" :data-id="index">
				...
				<view class="number" :class="{'classTop': index == 2 && item.num > 0, 'classEnd': index ==2 && item.num < 0}">{{ item.num }}名</view>
			</view>
		</scroll-view>
		...
		<view class="tableContent">
			<view class="title qiun-bg-white qiun-title-bar qiun-common-mt">
				<view class="qiun-title-student-light leftBlock margin20">一级指标的各维度统计表</view>
			</view>
			...
			<view class="testTable backFFF" :key="index" v-for="(item, index) in testTableData">
				...
				<view class="viewTitle">
					{{item.compareLastSemesterClassRank}}
				</view>
				<view class="viewTitle">
					{{item.specialtyRank}}
				</view>
		...
		-->
		<view class="title qiun-bg-white qiun-title-bar qiun-common-mt">
			<view class="qiun-title-student-light leftBlock margin20">本学期各维度统计雷达图</view>
		</view>
		...
		-->
		<view class="title qiun-bg-white qiun-title-bar qiun-common-mt">
			<view class="qiun-title-student-light leftBlock margin20">各学期发展趋势图</view>
		</view>
		<view class="qiun-charts" >
...
<script>
	...
	export default {
		...
		methods: {
			...
			async getSelfTest() {
				this.LoadProgress();
				const url = this.address.student + this.api.student.getSelfTest;
				...
				this.studentData = [
					{
						name: '总分数的班级排名',
						num: result.data.classRank
					},
					{
						name: '班级排名较上学期',
						num: result.data.classRankCompareLastSemester
					},
					{
						name: '总分数在专业排名',
						num: result.data.specialtyRank
					},
					{
						name: '总分数在本系排名',
				...
				// 雷达图
				let radarData = {
					categories: result.data.dimensionNameList,
					series: [{
						name: '个人',
						color: this.colorArray[0],
						data: result.data.dimensionValueList
					}]
				};
				...
				for(let i of result.data.lineSemesterList) {
					chartData.categories.push(`第${i}学期`)
				}
				result.data.lineDataList.forEach((i, index) => {
